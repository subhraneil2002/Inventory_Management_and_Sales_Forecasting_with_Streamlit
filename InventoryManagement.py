...
# Function to load data with multiple encodings and caching to improve speed
@st.cache_data
...
# Cache function for extracting columns
@st.cache_data
def extract_columns(df):
    required_columns = ['Year', 'Month', 'Quantity', 'Material Description']
    return df[required_columns]

# Cache function for cleaning the dataframe
@st.cache_data
...
# Function for creating lag features (not cached as it depends on user inputs)
...
# Cache function for tuning XGBoost to avoid re-running if data and parameters remain the same
@st.cache_data
...
# Forecast function (not cached since it dynamically uses last known data)
...
# Function for plotting forecast
