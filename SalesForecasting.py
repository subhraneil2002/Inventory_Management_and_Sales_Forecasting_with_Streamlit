...
# Optimized XGBoost model tuning function with RandomizedSearchCV
@st.cache_data
def tune_xgboost(X_train, y_train):
    tscv = TimeSeriesSplit(n_splits=3)  # Reduced splits for faster tuning
    xgb_param_grid = {
        'n_estimators': [100, 500],  # Reduced grid
        'max_depth': [3, 5],         # Limiting depth
        'learning_rate': [0.05, 0.1],  # Fewer options
        'subsample': [0.8]            # Reduced subsample
    }

    xgb_model = XGBRegressor(objective='reg:squarederror', early_stopping_rounds=10)
    xgb_random_search = RandomizedSearchCV(xgb_model, xgb_param_grid, cv=tscv,
